...
if(node.isFraction){return true;}break;case"mixedFraction":if(node.isMixedFraction){return true;}break;case"fractionOrDecimal":if(node.isFraction||node.isMixedFraction||node.numberFormat==="decimal"){return true;}break;default:(0,_assert.assert)(false,(0,_assert.message)(2015,[code]));break;}});}function checkMatrixType(fmt,node){var fmtList=normalizeFormatObject(fmt);return fmtList.some(function(f){var code=f.code;var length=f.length;switch(code){case"simpleSmallRowMatrix":case"smallRowMatrix":return node.op===_model.Model.MATRIX&&node.m===1&&node.n<4;case"simpleSmallColumnMatrix":case"smallColumnMatrix":return node.op===_model.Model.MATRIX&&node.m<4&&node.n===1;case"simpleSmallMatrix":case"smallMatrix":return node.op===_model.Model.MATRIX&&node.m<4&&node.n<4;case"matrix":return node.op===_model.Model.MATRIX;case"row":return node.op===_model.Model.ROW;case"column":return node.op===_model.Model.COL;default:return false;}});}function isSimpleExpression(node){if(node.op===_model.Model.NUM||node.op===_model.Model.VAR||typeof node==="string"){return true;}return false;}function hasSimpleExpressions(node){(0,_assert.assert)(node.op===_model.Model.MATRIX||node.op===_model.Model.ROW||node.op===_model.Model.COL);return(0,_backward.every)(node.args,function(n){if(n.op===_model.Model.MATRIX||n.op===_model.Model.ROW||n.op===_model.Model.COL){return hasSimpleExpressions(n);}return isSimpleExpression(n);});}function matchType(pattern,node){var types=_model.Model.option("types");if(pattern.op===_model.Model.TYPE&&pattern.args[0].op===_model.Model.VAR){var name=pattern.args[0].args[0];switch(name){case"number":case"integer":case"decimal":case"scientific":case"fraction":case"simpleFraction":case"mixedFraction":case"fractionOrDecimal":return checkNumberType(pattern.args[0],node);case"variable":return node.op===_model.Model.VAR;case"simpleSmallRowMatrix":case"simpleSmallColumnMatrix":case"simpleSmallMatrix":return checkMatrixType(pattern.args[0],node)&&hasSimpleExpressions(node);case"smallRowMatrix":case"smallColumnMatrix":case"smallMatrix":case"matrix":case"row":case"column":return checkMatrixType(pattern.args[0],node);default:var type=types[name];if(type){(0,_assert.assert)(type instanceof Array);return type.some(function(pattern){// FIXME pre-compile types.
...
// }
return matches;}function expandBinary(str,args){var t=str;(0,_backward.forEach)(args,function(arg,i){str=str.replace("%"+(i+1),arg.args[0]);});if(args.length>2){return expandBinary(t,[newNode(_model.Model.VAR,[str])].concat(args.slice(2)));}return str;}function expand(template,args,env){// Use first matched template for now.
var str=template.str;if(str&&args){var count=str.split("%").length-1;if(str.indexOf("%%")>=0){str=str.replace("%%",args[0].args[0]);}if(str.indexOf("%*")>=0){(function(){var s="";(0,_backward.forEach)(args,function(arg){if(s!==""){s+=" ";}// Replicate template for each argument.
s+=str.replace("%*",arg.args[0]).replace("%M",arg.m).replace("%N",arg.n);});str=s;// Overwrite str.
})();}if(str.indexOf("%M")>=0){(0,_assert.assert)(env.m);str=str.replace("%M",env.m);}if(str.indexOf("%N")>=0){(0,_assert.assert)(env.n);str=str.replace("%N",env.n);}if(count===2&&args.length>2){str=expandBinary(str,args);}else{(0,_backward.forEach)(args,function(arg,i){str=str.replace("%"+(i+1),arg.args[0]);});}return{op:_model.Model.VAR,args:[str]};}(0,_assert.assert)(false);return{op:_model.Model.VAR,args:[""]};}function getPrec(op){switch(op){case _model.Model.OR:return 1;case _model.Model.AND:return 2;case _model.Model.EQ:case _model.Model.NE:return 3;case _model.Model.LT:case _model.Model.GT:case _model.Model.LE:case _model.Model.GE:case _model.Model.NGTR:case _model.Model.NLESS:case _model.Model.NI:case _model.Model.SUBSETEQ:case _model.Model.SUPSETEQ:case _model.Model.SUBSET:case _model.Model.SUPSET:case _model.Model.NNI:case _model.Model.NSUBSETEQ:case _model.Model.NSUPSETEQ:case _model.Model.NSUBSET:case _model.Model.NSUPSET:return 4;case _model.Model.ADD:case _model.Model.SUB:case _model.Model.PM:return 5;case _model.Model.MUL:case _model.Model.FRAC:case _model.Model.DIV:case _model.Model.MOD:case _model.Model.COLON:return 6;case _model.Model.POW:case _model.Model.LOG:default:return 7;}(0,_assert.assert)(false,"ERROR missing precedence for "+op);}function isLowerPrecedence(n0,n1){// Is n1 lower precedence than n0?
...
matchedTemplates.push({str:""});}// Use first match.
...
(0,_assert.assert)(typeof template.str==="string");var str=template.str;if(str.indexOf("%%")>=0){return[node];}// let a = str.split("%");  // ["..", "1..", "2.."]
// let nn = a.filter(n => {
//   // Include '%1', %M, %N
//   return !isNaN(+n[0]) || n[0] === "M" || n[0] === "N";
// });
return node.args;}function translate(root,rules){// Translate math from LaTeX to English.
// rules = {ptrn: tmpl, ...};
...
args=args.concat(translate(n,[globalRules,argRules]));});return expand(template,args);},comma:function comma(node){if(node.op===_model.Model.MATRIX||node.op===_model.Model.ROW||node.op===_model.Model.COL){var _ret2=function(){var env={};if(node.op===_model.Model.MATRIX){(0,_assert.assert)(node.args[0].op===_model.Model.ROW);(0,_assert.assert)(node.args[0].args[0].op===_model.Model.COL);node.m=env.m=node.args[0].args.length;node.n=env.n=node.args[0].args[0].args.length;(0,_backward.forEach)(node.args,function(n,i){// matrix dimensions
n.m=i+1;});}else if(node.op===_model.Model.ROW){(0,_backward.forEach)(node.args,function(n,i){n.m=i+1;n.n=undefined;});}else{(0,_backward.forEach)(node.args,function(n,i){n.m=node.m;n.n=i+1;});}var matches=match(patterns,node);if(matches.length===0){return{v:node};}// Use first match for now.
var template=matchedTemplate(rules,matches,node.args.length);var args=[];var argRules=getRulesForArgs(template,rules);var nodeArgs=getNodeArgsForTemplate(node,template);(0,_backward.forEach)(nodeArgs,function(n,i){args=args.concat(translate(n,[globalRules,argRules]));args[i].m=n.m;args[i].n=n.n;});return{v:expand(template,args,env)};}();if((typeof _ret2==="undefined"?"undefined":_typeof(_ret2))==="object")return _ret2.v;}else{var _ret3=function(){var matches=match(patterns,node);if(matches.length===0){return{v:node};}// Use first match for now.
var template=matchedTemplate(rules,matches,node.args.length);var argRules=getRulesForArgs(template,rules);var nodeArgs=getNodeArgsForTemplate(node,template);var args=[];(0,_backward.forEach)(nodeArgs,function(n,i){args=args.concat(translate(n,[globalRules,argRules]));});return{v:expand(template,args)};}();if((typeof _ret3==="undefined"?"undefined":_typeof(_ret3))==="object")return _ret3.v;}},equals:function equals(node){var matches=match(patterns,node);if(matches.length===0){return node;}// Use first match for now.
var template=matchedTemplate(rules,matches,node.args.length);var argRules=getRulesForArgs(template,rules);var nodeArgs=getNodeArgsForTemplate(node,template);var args=[];(0,_backward.forEach)(nodeArgs,function(n,i){args=args.concat(translate(n,[globalRules,argRules]));});return expand(template,args,node);},paren:function paren(node){//assert (node.args.length === 1);
...
// LN: "ln",
// LG: "lg",
VAR:"var",NUM:"num",CST:"cst",COMMA:",",POW:"^",SUBSCRIPT:"_",ABS:"abs",PAREN:"()",HIGHLIGHT:"hi",LT:"lt",LE:"le",GT:"gt",GE:"ge",NE:"ne",NGTR:"ngtr",NLESS:"nless",NI:"ni",SUBSETEQ:"subseteq",SUPSETEQ:"supseteq",SUBSET:"subset",SUPSET:"supset",NNI:"nni",NSUBSETEQ:"nsubseteq",NSUPSETEQ:"nsupseteq",NSUBSET:"nsubset",NSUPSET:"nsupset",APPROX:"approx",IMPLIES:"implies",RIGHTARROW:"rightarrrow",LEFTARROW:"leftarrrow",LONGRIGHTARROW:"longrightarrow",LONGLEFTARROW:"longleftarrow",OVERRIGHTARROW:"overrightarrow",OVERLEFTARROW:"overleftarrow",LONGLEFTRIGHTARROW:"longleftrightarrow",OVERLEFTRIGHTARROW:"overleftrightarrow",PERP:"perp",PROPTO:"propto",PARALLEL:"parallel",NPARALLEL:"nparallel",SIM:"sim",CONG:"cong",INTERVAL:"interval",LIST:"list",SET:"set",EXISTS:"exists",IN:"in",FORALL:"forall",LIM:"lim",EXP:"exp",TO:"to",SUM:"sum",PIPE:"pipe",INT:"int",PROD:"prod",CUP:"cup",BIGCUP:"bigcup",CAP:"cap",BIGCAP:"bigcap",PERCENT:"%",QMARK:"?",M:"M",FACT:"fact",BINOM:"binom",ROW:"row",COL:"col",COLON:"colon",MATRIX:"matrix",TYPE:"type",OVERSET:"overset",UNDERSET:"underset",OVERLINE:"overline",DEGREE:"degree",BACKSLASH:"backslash",MATHBF:"mathbf",DOT:"dot",MATHFIELD:"mathfield",NONE:"none"};(0,_backward.forEach)((0,_backward.keys)(OpStr),function(v,i){Model[v]=OpStr[v];});var OpToLaTeX={};OpToLaTeX[OpStr.ADD]="+";OpToLaTeX[OpStr.SUB]="-";OpToLaTeX[OpStr.MUL]="\\times";OpToLaTeX[OpStr.DIV]="\\div";OpToLaTeX[OpStr.FRAC]="\\frac";OpToLaTeX[OpStr.EQL]="=";OpToLaTeX[OpStr.ATAN2]="\\atan2";OpToLaTeX[OpStr.POW]="^";OpToLaTeX[OpStr.SUBSCRIPT]="_";OpToLaTeX[OpStr.PM]="\\pm";OpToLaTeX[OpStr.NOT]="\\not";// OpToLaTeX[OpStr.SIN] = "\\sin";
...
if(hd()===TK_UNDERSCORE){next({oneCharToken:true});args.push(primaryExpr());if(hd()===TK_CARET){eat(TK_CARET,{oneCharToken:true});args.push(primaryExpr());}}if(hd()){args.push(commaExpr());}// Finish the log function
return newNode(tokenToOperator[tk],args);break;case TK_EXISTS:next();return newNode(Model.EXISTS,[equalExpr()]);case TK_FORALL:case TK_RIGHTARROW:case TK_LEFTARROW:case TK_LONGRIGHTARROW:case TK_LONGLEFTARROW:case TK_OVERRIGHTARROW:case TK_OVERLEFTARROW:case TK_LONGLEFTRIGHTARROW:case TK_OVERLEFTRIGHTARROW:next();return newNode(tokenToOperator[tk],[commaExpr()]);case TK_EXP:next();return newNode(Model.EXP,[additiveExpr()]);case TK_M:next();return newNode(Model.M,[multiplicativeExpr()]);case TK_OVERLINE:case TK_DOT:case TK_MATHFIELD:next();return newNode(tokenToOperator[tk],[braceExpr()]);case TK_OVERSET:case TK_UNDERSET:next();expr1=braceExpr();expr2=braceExpr();// Add the annotation to the variable.
...
function postfixExpr(){var t=void 0;var expr=exponentialExpr();switch(t=hd()){case TK_PERCENT:next();expr=newNode(Model.PERCENT,[expr]);break;case TK_BANG:next();expr=newNode(Model.FACT,[expr]);break;default:if(t===TK_VAR&&lexeme()==="\\degree"){next();if(hd()===TK_VAR&&(lexeme()==="K"||lexeme()==="C"||lexeme()==="F")){expr=multiplyNode([expr,unaryNode(Model.VAR,["\\degree "+lexeme()])]);next();}else{expr=multiplyNode([expr,unaryNode(Model.VAR,["\\degree"])]);}}else if(t===TK_VERTICALBAR&&lookahead()===TK_UNDERSCORE){// x|_{x=3}, x|_1^2
next();var args=[expr];next({oneCharToken:true});args.push(newNode(Model.SUBSCRIPT,[equalExpr()]));expr=newNode(Model.PIPE,args);}else if(isChemCore()&&(t===TK_ADD||t===TK_SUB)&&lookahead()===TK_RIGHTBRACE){next();// 3+, ion
...
function isOneOrMinusOne(node){return isOne(node)||isMinusOne(node);}function isOne(node){return node.op===Model.NUM&&node.args[0]==="1";}function isMinusOne(node){return node.op===Model.SUB&&node.args.length===1&&isOne(node.args[0]);}function multiplicativeExpr(){var t,expr,explicitOperator=false,isFraction,args=[];var n0;expr=fractionExpr();if(expr.op===Model.MUL&&!expr.isBinomial&&expr.args[expr.args.length-1].op!==Model.VAR&&expr.args[expr.args.length-1].args[0]==="\\degree"){// FIXME binomials and all other significant syntax should not be desugared
...
if(isOne(args[0])){args.pop();}else{expr=negate(expr);}}else if(args.length>0&&(n0=isRepeatingDecimal([args[args.length-1],expr]))){args.pop();expr=n0;}else if(expr.op===Model.VAR&&expr.args[0].indexOf("'")===0){var t=args.pop();expr=binaryNode(Model.MUL,[t,expr]);expr.isImplicit=true;}else if(!isChemCore()&&isPolynomialTerm(args[args.length-1],expr)){// 2x, -3y but not CH (in chem)
...
//   }
// } else
if(!args[0].lbrk&&(args[0].op===Model.NUM&&args[0].numberFormat==="decimal"||args[0].op===Model.VAR&&args[0].args[0]==="?"||args[0].op===Model.TYPE&&args[0].args[0].op===Model.VAR&&args[0].args[0].args[0]==="decimal")){// No lbrk so we are in the same number literal.
if(args[1].lbrk===40&&(isInteger(args[1])||args[1].op===Model.TYPE&&args[1].args[0].op===Model.VAR&&args[1].args[0].args[0]==="integer")){n0=args[0];n1=args[1];}else if(!args[1].lbrk&&args[1].op===Model.OVERLINE){//   // 3.\overline{12} --> 3.0+(0.12, repeating)
...
function additiveExpr(){var expr=multiplicativeExpr();var t=void 0;while(isAdditive(t=hd())){next();var expr2=multiplicativeExpr();switch(t){case TK_BACKSLASH:expr=binaryNode(Model.BACKSLASH,[expr,expr2]);break;case TK_PM:expr=binaryNode(Model.PM,[expr,expr2]);break;case TK_SUB:expr=binaryNode(Model.SUB,[expr,expr2]);break;default:expr=binaryNode(Model.ADD,[expr,expr2]);break;}}return expr;}//
...
expr=Model.create(expr2);}if(args.length===0){expr=expr;}else if(args.length===1){expr=args[0];}else{expr=newNode(Model.COMMA,args);}return expr;}// Parse 'x = 10'
function isEquality(t){return t===TK_EQL||t===TK_NE||t===TK_APPROX;}function equalExpr(){var expr=relationalExpr();var t=void 0;var args=[];while(isEquality(t=hd())){// x = y = z -> [x = y, y = z]
next();var expr2=additiveExpr();expr=newNode(tokenToOperator[t],[expr,expr2]);}return expr;}function isImplies(t){return t===TK_IMPLIES||t===TK_RIGHTARROW||t===TK_LEFTARROW||t===TK_LONGRIGHTARROW||t===TK_LONGLEFTARROW||t===TK_OVERRIGHTARROW||t===TK_OVERLEFTARROW||t===TK_LONGLEFTRIGHTARROW||t===TK_OVERLEFTRIGHTARROW||t===TK_VERTICALBAR;}function impliesExpr(){var expr=equalExpr();var t=void 0;var args=[];while(isImplies(t=hd())){next();var expr2=equalExpr();expr=newNode(tokenToOperator[t],[expr,expr2]);}return expr;}// Parse 'a, b, c, d'
function commaExpr(){var expr=impliesExpr();var args=[expr];var t=void 0;while((t=hd())===TK_COMMA){next();args.push(impliesExpr());}if(args.length>1){expr=newNode(tokenToOperator[TK_COMMA],args);}return expr;}// Root syntax.
...
"\\right":null,"\\big":null,"\\Big":null,"\\bigg":null,"\\Bigg":null,"\\ ":null,"\\quad":null,"\\qquad":null,"\\text":TK_TEXT,"\\textrm":TK_TEXT,"\\textit":TK_TEXT,"\\textbf":TK_TEXT,"\\lt":TK_LT,"\\le":TK_LE,"\\gt":TK_GT,"\\ge":TK_GE,"\\ne":TK_NE,"\\neq":TK_NE,"\\ngtr":TK_NGTR,"\\nless":TK_NLESS,"\\ni":TK_NI,"\\subseteq":TK_SUBSETEQ,"\\supseteq":TK_SUPSETEQ,"\\subset":TK_SUBSET,"\\supset":TK_SUPSET,"\\approx":TK_APPROX,"\\implies":TK_IMPLIES,"\\rightarrow":TK_RIGHTARROW,"\\leftarrow":TK_LEFTARROW,"\\longrightarrow":TK_LONGRIGHTARROW,"\\longleftarrow":TK_LONGLEFTARROW,"\\overrightarrow":TK_OVERRIGHTARROW,"\\overleftarrow":TK_OVERLEFTARROW,"\\longleftrightarrow":TK_LONGLEFTRIGHTARROW,"\\overleftrightarrow":TK_OVERLEFTRIGHTARROW,"\\perp":TK_PERP,"\\propto":TK_PROPTO,"\\parallel":TK_PARALLEL,"\\nparallel":TK_NPARALLEL,"\\sim":TK_SIM,"\\cong":TK_CONG,"\\exists":TK_EXISTS,"\\in":TK_IN,"\\forall":TK_FORALL,"\\lim":TK_LIM,"\\exp":TK_EXP,"\\to":TK_TO,"\\sum":TK_SUM,"\\int":TK_INT,"\\prod":TK_PROD,"\\cup":TK_CUP,"\\bigcup":TK_BIGCUP,"\\cap":TK_CAP,"\\bigcap":TK_BIGCAP,"\\%":TK_PERCENT,"\\binom":TK_BINOM,"\\begin":TK_BEGIN,"\\end":TK_END,"\\colon":TK_COLON,"\\vert":TK_VERTICALBAR,"\\lvert":TK_VERTICALBAR,"\\rvert":TK_VERTICALBAR,"\\mid":TK_VERTICALBAR,"\\type":TK_TYPE,"\\overline":TK_OVERLINE,"\\overset":TK_OVERSET,"\\underset":TK_UNDERSET,"\\backslash":TK_BACKSLASH,"\\mathbf":TK_MATHBF,"\\abs":TK_ABS,"\\dot":TK_DOT,"\\MathQuillMathField":TK_MATHFIELD,"\\ldots":TK_VAR,// ... and var are close syntactic alternatives
"\\vdots":TK_VAR,"\\ddots":TK_VAR};var identifiers=(0,_backward.keys)(env);// Start scanning for one token.
...
case 36:// dollar
_lexeme+=String.fromCharCode(c);return TK_VAR;case 39:// prime (single quote)
return prime(c);case 60:// left angle
if(src.charCodeAt(curIndex)===61){// equals
