...
function isRelational(t){return t===TK_LT||t===TK_LE||t===TK_GT||t===TK_GE||t===TK_IN||t===TK_TO||t===TK_PERP||t===TK_PROPTO||t===TK_NGTR||t===TK_NLESS||t===TK_NI||t===TK_NOT||t===TK_SUBSETEQ||t===TK_SUPSETEQ||t===TK_SUBSET||t===TK_SUPSET;}// Parse 'x < y'
...
while(c&&c!=="{".charCodeAt(0)){c=src.charCodeAt(curIndex++);}_lexeme="";c=src.charCodeAt(curIndex++);while(c&&c!=="}".charCodeAt(0)){var ch=String.fromCharCode(c);if(ch==="&"&&(0,_backward.indexOf)(src.substring(curIndex),"nbsp;")===0){// Skip &nbsp;
curIndex+=5;}else if(ch===" "||ch==="\t"){// Skip space and tab
}else{_lexeme+=ch;}c=src.charCodeAt(curIndex++);}if(tk!==TK_TYPE){// Not a type, so convert to a var.
