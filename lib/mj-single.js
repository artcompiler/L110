    ...
    AuthorInit: function AuthorInit() {
      ...
      MathJax.Hub.Register.StartupHook("SVG Jax Config", function () {
        ...
        SVG.Augment({
          ...
          //
          InitializeSVG: function InitializeSVG() {
            this.defaultEx = 6;
          ...
          //
          preTranslate: function preTranslate(state) {
        ...
        SVG.BBOX.TEXT.Augment({
          Init: function Init(scale, text, def) {
            if (!def) {
              def = {};
            };def.stroke = "none";
...
function Insert(dst, src) {
  for (var id in src) {
    if (src.hasOwnProperty(id)) {
      ...
      if (_typeof(src[id]) === 'object' && !(src[id] instanceof Array) && (_typeof(dst[id]) === 'object' || typeof dst[id] === 'function')) {
...
function GetSVG(result) {
  ...
  //
  var svgfile = ['<?xml version="1.0" standalone="no"?>', '<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">', svgdata].join("\n");
  ...
  if (data.img) {
    ...
    result.img = ['<img src="file.svg" style="', svg.style.cssText, " width:", svg.getAttribute("width"), "; height:", svg.getAttribute("height"), ';"', data.speakText ? ' alt="' + result.speakText + '"' : "", ' />'].join("");
  }
}
...
function GetPNG(result) {
  ...
  if (data.png) {
    ...
    var check = function check(err) {
      if (err) {
        AddError(err.message);synch();return true;
      }
    };
...
function StartQueue() {
  ...
  var math = data.math;
  if (data.format !== "MathML") math = math.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  ...
  HUB.Queue($$("Typeset", HUB), $$(TypesetDone, result), $$(GetMML, result), $$(GetSpeech, result), $$(GetSVG, result), $$(GetPNG, result), $$(ReturnResult, result));
}
...
exports.typeset = function (data, callback) {
  ...
  if (!delimiters[options.format]) {
    ReportError("Unknown format: " + options.format, callback);return;
  }
